...
namespace Godless_Lands_Game.Professions.Components
{
    ...
    public class ProfessionsDbSyncComponent : Component, IDatabaseReadable, IDatabaseWritable
    {
        ...
        {
            ProfessionDbData[] professionDbDatas = await JobsManager.Execute(GameDatabase.Provider.Select<ProfessionDbData[]>($"SELECT load_professions('{_characterInfoHolder.CharacterID}')"));
        ...
        public async Job<bool> WriteToDatabase()
        {
            try
            {
                foreach (Profession profession in _professions.Professions)
                {
                    ...
                    {
                        await JobsManager.Execute(GameDatabase.Provider.Call($"CALL save_profession('{_characterInfoHolder.CharacterID}', {(int)profession.ProfessionType}, {profession.Level}, {profession.Experience})"));
