...
namespace Game.Equipment.Components
{
    internal class EquipmentDbSyncComponent : Component, IDatabaseReadable, IDatabaseWritable
    {
        ...
        {
            _items = await JobsManager.Execute(GameDatabase.Provider.Select<List<EquipmentDbData>>($"SELECT load_equipment('{_characterInfoHolder.CharacterID}')"));
        }
        ...
        public async Job<bool> WriteToDatabase()
        {
            ...
            foreach (var equipmentSlot in _equipment.Items)
            {
                ...
                jobs.Add(
                JobsManager.Execute(GameDatabase.Provider.Call($"CALL save_equipment('{_characterInfoHolder.CharacterID}', {(int)equipmentSlot.EquipmentType}, {equipmentSlot.Item?.UniqueID ?? 0})")));
            }
