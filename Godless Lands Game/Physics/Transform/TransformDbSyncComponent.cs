...
namespace Game.Physics.Transform
{
    public class TransformDbSyncComponent : TransformComponent, IDatabaseReadable, IDatabaseWritable
    {
        ...
        {
            m_position = await JobsManager.Execute(GameDatabase.Provider.Select<Vector3>($"SELECT get_chatacer_position('{m_characterInfoHolder.CharacterID}')"));
        ...
        {
            _lastSaveVersion = m_version;
            return await JobsManager.Execute(GameDatabase.Provider.Call($"CALL set_character_position('{m_characterInfoHolder.CharacterID}', '{FloatHelper.FloatToString(m_position.X)}', '{FloatHelper.FloatToString(m_position.Y)}', '{FloatHelper.FloatToString(m_position.Z)}')"));
        }
    }
}
