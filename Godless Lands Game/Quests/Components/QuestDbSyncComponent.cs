...
namespace Godless_Lands_Game.Quests.Components
{
    ...
    public class QuestDbSyncComponent : Component, IDatabaseReadable, IDatabaseWritable
    {
        ...
        {
            QuestDbData[] questDbDatas = await JobsManager.Execute(GameDatabase.Provider.Select<QuestDbData[]>($"SELECT load_quests('{_characterInfoHolder.CharacterID}')"));
        ...
        public async Job<bool> WriteToDatabase()
        {
            try
            {
                foreach (Quest quest in _questController.Quests)
                {
                    ...
                    {
                        await JobsManager.Execute(GameDatabase.Provider.Call($"CALL save_quest('{_characterInfoHolder.CharacterID}', {(int)quest.ID}, {quest.CurrentStageID})"));
                        quest.MarkDataAsSyncedWithDb();
                    }
                }
            }
