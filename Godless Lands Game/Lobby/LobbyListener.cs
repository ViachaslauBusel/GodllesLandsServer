    ...
    class LobbyListener
    {
        ...
        public static async void Entrance(PlayerProfile profile, Packet packet)
        {
           // packet.Read(out MSG_LOBBY_ENTRANCE_CS request);
            ...
            string charactersJsonData = await GameDatabase.Provider.SelectJson($"SELECT get_characters('{profile.AuthorizationHolder.LoginID}')");
        ...
        public static async void SelectCharacter(PlayerProfile profile, Packet packet)
        {
            ...
            if (await GameDatabase.Provider.SelectObject($"SELECT is_character_belong('{profile.AuthorizationHolder.LoginID}', '{request.CharacterID}')") is bool isBelong && isBelong && profile.CharacterObjectID == 0)
